...
import { fetchFacebookHub, fetchGoogleHub, fetchZaloHub, staffMe, staffRefresh } from '@/lib/api';
import {
  clearSession,
  getAccessToken,
  getRefreshToken,
...
function AdsCombinedContent() {
  ...
  const [zalo, setZalo] = useState<Record<string, unknown> | null>(null);
    ...
    void (async () => {
      ...
      try {
        const [m, g, z] = await Promise.all([
          ...
          fetchZaloHub(access, { days: 7 }).catch(() => null),
        ]);
        setMeta(m?.summary ?? null);
        setGoogle(g?.summary ?? null);
        setZalo(z?.summary ?? null);
      } catch (err) {
  ...
  return (
    <main style={{ maxWidth: 1000, margin: '0 auto', padding: '1.5rem' }}>
      ...
      <h1 style={{ fontSize: '1.25rem' }}>Ads CPL — Meta + Google + Zalo</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
        ...
        <div className="card">
          ...
          <p className="muted">Spend: {fmtVnd(Number(zalo?.total_spend ?? 0))}</p>
          <p className="muted">Leads: {String(zalo?.total_leads ?? 0)} · CPL TB: {fmtVnd(zalo?.avg_cpl as number)}</p>
          <Link href="/zalo/zalo-ads" className="nav-link">
            Mở Zalo hub →
          </Link>
        </div>
      </div>
