...
export function ZaloZaloAdsContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const [user, setUser] = useState<StoredStaffUser | null>(null);
  const [hub, setHub] = useState<ZaloHubResponse | null>(null);
  ...
  const ensureAuth = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (!ok) {
        setError('Không có quyền Zalo hub');
        return null;
      }
      return access;
    } catch {
  ...
  const syncUrl = useCallback(() => {
    ...
    router.replace(suffix ? `/zalo/zalo-ads?${suffix}` : '/zalo/zalo-ads', {
      scroll: false,
    });
  ...
  const loadHub = useCallback(
    async (access: string) => {
      ...
      setError('');
      try {
        const data = await fetchZaloHub(access, hubQuery);
        setHub(data);
        syncUrl();
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Tải Zalo hub thất bại');
      } finally {
        setLoading(false);
      }
    },
            ...
            <tbody>
              {rows.map((c) => (
                <tr key={c.id}>
                  ...
                  <td>{c.token_status ?? (c.zalo_has_token ? 'ok' : '—')}</td>
                </tr>
              ))}
